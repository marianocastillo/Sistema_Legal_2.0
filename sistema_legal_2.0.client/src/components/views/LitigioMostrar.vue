  ...
  <div class="card p-6 shadow-2">
    ...
    <div v-else>
      ...
      <!-- Audiencias con evidencias y comentarios -->
        <Accordion :activeIndex="null" class="mb-5 section-title">
          <AccordionTab header="Audiencias y Evidencias" class="audiencias-tab">
            <Accordion v-model:activeIndex="activeIndex" multiple class="accordion-sin-flecha fondo-header">
              <AccordionTab v-for="(audiencia, index) in audiencias" :key="index">
                ...
                <!-- Contenido del AccordionTab de evidenciass-->
...
.audiencias-tab ::v-deep(.p-accordion-header) {
  color: white;
}
...
:deep(.fondo-header .p-accordion .p-accordionpanel-active > .p-accordionheader) {
  --p-accordion-header-active-background: #e0dddd !important;
...
::v-deep(.qalendar-header-custom) {
  ...
  gap: 1rem;
  flex-wrap: wrap;
  /* para que no rompa en pantallas chicas */
}
