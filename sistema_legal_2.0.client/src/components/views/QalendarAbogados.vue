...
const eventos = ref([])
const usuario = ref({ nombre: '', rol: '', perfil: null, idUsuario:null });
...
onMounted(async () => {
   ...
   if (stored) {
    ...
    const id_usuario = usuario.value.idUsuario;
  window.__goToDetalle = (id) => {
    router.push(`/Detalles/${id}`)
  }

  try {
    const { data } = await axios.get(`/api/Files/AudienciasCalendarios/${id_usuario}`)
